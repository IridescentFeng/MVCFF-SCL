...
def resnet50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    """
    return _resnet('resnet50', Bottleneck, [3, 4, 6],
                   **kwargs)
...
def resnext50(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    return _resnet('resnext50_32x4d', Bottleneck, [3, 4, 6], **kwargs)
...
class BCLModel(nn.Module):
    def __init__(self, num_classes=100, name='resnext50', head='mlp', use_norm=True, feat_dim=100,norm_layer: Optional[Callable[..., nn.Module]] = None):
        ...
        self.layer4 = self._make_layer(Bottleneck, 512, 3, stride=2,
                                                 dilate=False)
        self.indivdual = nn.ModuleList([self._make_layer(Bottleneck, 512, 3, stride=2,
                                                 dilate=False) for _ in range(3)])
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        #self.encoder = model_fun()
        self.batch_size = 32
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(1024, planes * block.expansion, stride),
                norm_layer(planes * block.expansion),
            )

        layers = []
        layers.append(block(1024, 512, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
