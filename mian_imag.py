...
parser.add_argument('--data', default='/root/autodl-tmp', metavar='DIR')
...
parser.add_argument('--feat_dim', default=1000, type=int, choices=[100, 10, 1000],help='feature dimension of mlp head')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    '''train_dataset = IMBALANCECIFAR100(root=args.data, imb_type='exp', imb_factor=0.01, train=True, transform=transform_train,
                                     download=True)
    val_dataset = IMBALANCECIFAR100(root=args.data, imb_type='exp', imb_factor=0.01, train=False, transform=val_transform,
                                   download=True)'''
    ...
    confusion = ConfusionMatrix(num_classes=args.feat_dim, labels=train_dataset.labels)
    ...
    if args.reload:
        ...
        test_loader = torch.utils.data.DataLoader(
            ...
            num_workers=args.workers, pin_memory=True)
        acc1, acc1_avg, many, med, few, pre, recall, f1 = validate(train_loader, val_loader, model, criterion_ce, 1, args, confusion, tf_writer)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion_ce, criterion_scl, optimizer, epoch, gradients, args, tf_writer)

        # evaluate on validation set
        acc1_avg, many, med, few = validate(train_loader, val_loader, model, criterion_ce, epoch, args,
                                                         tf_writer)
...
def train(train_loader, model, criterion_ce, criterion_scl, optimizer, epoch, gradients, args, tf_writer):
